...
programs_eezs <- program_cells %>%
  left_join(eez_cells_g7, by = "h3") %>%
  filter(!is.na(eez))

programs_eezs_stats <- programs_eezs %>%
  group_by(eez) %>%
...
programs_time <- purrr::map(programs_json, function(program) {
  start <- NA
  end <- Sys.Date()
  active <- TRUE
...
ggplot() +
  geom_segment(programs_time, mapping = aes(y = title, yend = title, x = start, xend = end, color = active), size = 2) +
